...
export const Route = createFileRoute("/practica")({
  head: () => ({
    meta: [
      { title: "Práctica guiada — AD PrivEsc Lab" },
      {
        name: "description",
        content:
          "Analiza salidas simuladas (whoami /priv, Kerberos, ACL) y reconoce el vector correcto.",
      },
      { property: "og:title", content: "Práctica guiada — AD PrivEsc Lab" },
      {
        property: "og:description",
        content: "Entrena reconocimiento de firmas de privilege escalation en lab.",
      },
    ],
  }),
...
function PracticaPage() {
  ...
  const chosenOption = useMemo(
    () => scenario.options.find((o) => o.vectorId === chosen) ?? null,
    [scenario, chosen],
  );
    ...
    <div className="mx-auto max-w-4xl space-y-8 pb-16 pt-6">
      <header className="space-y-4">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-4xl">
          Interpreta la firma y nombra el vector
        </h1>
        <p className="text-muted-foreground">
          Lee el escenario, mira la salida simulada y decide qué vector encaja. Ningún comando se
          ejecuta: es entrenamiento de reconocimiento para labs autorizados.
        </p>
            ...
            <div className="rounded-2xl border border-primary/25 bg-primary/5 p-5">
              ...
              <div className="mt-4 grid gap-2">
                {scenario.options.map((opt) => {
                  const isChosen = chosen === opt.vectorId;
                  const state = !chosen
                    ? "idle"
                    : isChosen
                      ? opt.correct
